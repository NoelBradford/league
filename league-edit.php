<form class="user" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" autocomplete="off">
	<?php $league->display_info(); ?>
	<?php $league->display_errors(); ?>
	<input type="hidden" name="old_email" <?php echo 'value="' . $league->get_email() . '"'; ?> />
	<label for="email">E-mail:</label>
	<input type="email" id="email" name="email" placeholder="email" <?php echo 'value="' . $league->get_email() . '"'; ?> />
